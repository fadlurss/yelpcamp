    ...
    function(req, email, password, done) {
        ...
        process.nextTick(function() {
            User.findOne({ 'local.email' :  email }, function(err, user) {
                ...
                var token = jwt.sign({id: user.local.id}, config.secret, {
                    expiresIn: 86400 //expires in 24 hours
                });
                    return done(null, user, req.flash('success','Welcome back to Yelpcamp, '+user.local.email+' This is your token '+token));
            });
        });
    ...
    function(req, email, password, done) {
        ...
        process.nextTick(function() {
            ...
            if (!req.user) {
                ...
                req.checkBody('password', 'Invalid password, min password length is 8').notEmpty().isLength({min:8, max:100});
                ...
                User.findOne({ 'local.email' :  email }, function(err, user) {
                    ...
                    if (user) {
                        ...
                        // eval(require('locus'))
                        if(req.body.adminCode === 'kode_admin_435934623'){
            ...
            if (!req.user) {
                ...
                User.findOne({ 'facebook.id' : profile.id }, function(err, user) {
                    ...
                    if (user) {
                        ...
                        return done(null, user, req.flash('success','Welcome back to Yelpcamp, '+user.facebook.name)); // user found, return that user
                    } else {
