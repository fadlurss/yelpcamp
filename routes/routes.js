...
// user account will stay active in case they want to reconnect in the future
    ...
    router.get("/forgot", function(req,res){
        Categories.find({}, function(err,categories){
            res.render("v_access/forgot", {categories:categories});
        });
    });
    ...
    router.post('/forgot', function(req, res, next) {
      ...
      router.get('/reset/:token', function(req, res) {
        User.findOne({ 'local.resetPasswordToken': req.params.token, 'local.resetPasswordExpires': { $gt: Date.now() } }, function(err, cari) {
          ...
          Categories.find({}, function(err,categories){
            res.render("v_access/reset", {token: req.params.token, categories:categories});
          });
    ...
    router.post('/updatepass', async (req, res) => {
        const user = await User.findOne({_id: "5bdebf5dbdb3f7cc8c0bd4af"})
    ...
    router.post('/reset/:token', function(req, res) {
        ...
        router.get('/verify/:tokenReg', function(req,res){
            User.findOne({'local.tokenReg' : req.params.tokenReg}, function(err, result){
                if(err){
                    console.log(err);
                    res.redirect("back");
                } else {
        ...
        router.get("/alluser", middleware.asyncMiddleware(async(req,res,next) => {
            ...
            res.render("v_access/alluser",{allUser:allUser});
        }));

        // UPDATE STATUS AKUN
        router.post('/statusAkun', middleware.asyncMiddleware(async(req,res,next) => {
            var userData = { "local.statusAkun" : req.body.statusAkun}
            const update_statusAkun = await User.findByIdAndUpdate(req.body.id, {$set: userData});
